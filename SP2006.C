 ...
 * (Diese Version des Programms läuft ohne Daten von RS232 zu empfangen. Der Wert der Temperatur
 * ist simuliert und kann mit den Tasten "q" und "a" verändert werden.)
 *
 * Autor: A. Schreiber
 * Date:  Oktober 2006
 **************************************************************************************************/
...
int ibyte=100;									/* Variable für Einlesen COM (Anfangswert testweise vorgegeben) */
	...
	if(kbhit())
		{
			...
			if (key!= 113)								/* bei "q" Wert um 1 erhöhen */
				ibyte--;
			if (key!= 97)								/* bei "a" Wert um 1 erniedrigen */
				ibyte++;
		}
	if((now-alt)/CLOCKS_PER_SEC>=1)						/* diese Schleife wird sekündlich ausgeführt */
	{
		twert=0;										/* Rücksetzen twert-Variable */
		outportb(com,150);								/* Ausgabe Wert über COM zum Testen */
		/* ibyte=inportb(com);							/* Einlesen von COM2 (nur mit Brücke möglich) */
